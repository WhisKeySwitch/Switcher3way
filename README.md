...
Build from source (no published releases; the app is unnotarized — first launch on another Mac needs right-click → **Open**):
...
On first launch the onboarding checklist asks for two macOS permissions — **Accessibility** (read and retype the mistyped word) and **Input Monitoring** (see keystrokes). Grants are detected live; the app restarts itself after Input Monitoring is granted. Local rebuilds are signed with a stable self-signed certificate (see `signing/README.md`) so the permissions survive rebuilds.
