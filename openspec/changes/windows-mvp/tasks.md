...
- [x] 5.2 Settings persistence (`SettingsManager` → `%AppData%\Switcher3way\settings.json`) + a **settings window** (`SettingsForm`, opened from the tray "Settings…"): feature toggles and editable exception lists. (Trigger-key config not yet exposed — F9 is fixed for now.)
- [~] 5.3 Exceptions: **denied-apps** list (with defaults — password managers, terminals, RDP), **never-convert** / **always-convert** word lists — all persisted in `SettingsManager` and enforced in `Engine` (denied-app gate on auto AND manual; never-convert on auto; always-convert via Core `IAlwaysConvertList`), and **editable in the settings window**. Secure/**password-field detection via UI Automation** still to add.
