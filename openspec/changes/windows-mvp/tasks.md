...
- [x] 5.2 Settings persistence (`SettingsManager` → `%AppData%\Switcher3way\settings.json`) + a **settings window** (`SettingsForm`, opened from the tray "Settings…"): feature toggles, a **manual trigger-key selector** (F8–F12 / Pause / Scroll Lock / Right Ctrl), and editable exception lists.
