...
**Updating from upstream** — the upstream updater was deleted at fork time so stock rashn/RuSwitcher releases could never clobber the fork. The fork's own updater (capability 11, July 2026) is scoped exclusively to the project's own releases — `WhisKeySwitch/Switcher3way` since the August 2026 consolidation (previously the separate `switcher3way-releases` downloads repo); upstream remains out of the update path by design.
